...
public class DeviceIoTHubService(IConfiguration config) : BackgroundService
{
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (true)
        {
            ...
            var message = new TelemetryMessagePayload
            {
                Timestamp = now,
                MessageType = "ua-deltaframe",
                Payload = new Dictionary<string, TelemetryMessagePayloadItem>
                {
                    ["State"] = new() { Timestamp = now, Value = "running" },
                    ["AlarmSubsystem1"] = new() { Timestamp = now, Value = true },
                    ["AlarmSubsystem2"] = new() { Timestamp = now, Value = false },
                    ["AbsorbedEnergy"] = new() { Timestamp = now, Value = Random.Shared.NextDouble() * 100 }
                },
                SourceName = cn,
